...
int tmp,s=0,dead=1,x,y=400,vy,wx[]={0,0},wy[]={0,0},e=1800,l=600;
void draw() { //RULES: Max 100 characters per line. 1 semicolon per line, not including for loops
  ...
  for(int i=0,z=y+=vy+=1,q=(x=x-6==-1800?0:x-6);i<2;i++,fill(0),textSize(40),image(b,l/2,y)){
    ...
    if((wx[i]=wx[i]<0?(wy[i]=(int)random(200,height-200))/wy[i]*l:wx[i]-6) == l/2) s++;
    dead = (abs(width/2-wx[i])<25 && abs(y-wy[i])>100) || y > height || y < 0 ? 1 : dead;
  } //MORE RULES: close curly brackets on their own line. Must run in Processing Java + JS
  if (dead==(tmp=1)) for(imageMode(CORNER);tmp>0;tmp--,rectMode(3)) image(m, 0, 0);
  if(s>hs&&dead==1){hs=s-1;}text(""+hs,l/2-15,750);
...
void mousePressed() { //Start learning Java for free: http://www.ktbyte.com/intro
  vy = -17; //And use this free textbook: http://www.ktbyte.com/textbook/intro
  if (dead==1) y=(wx[1]=900) + 100 - ((wy[0]=(wy[1]=wx[0]=600)-200)+200 + (x = s = dead = 0));
} //This is line 19. Flying is learning how to throw yourself at the ground and miss ~Douglas Adams
