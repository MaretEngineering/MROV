...
void draw() {
    ...
    if ((joy1y != 0 || joy1x != 0) && (joy2x == 0)) { //strafe mode
        thrustValues = getTranslation(joy1y, -joy1x);
        // Map thrust vectors to between -256 and +256
        for (int i = 0; i < 4; i++) {
            if (thrustValues[i] > 512 || thrustValues[i] < -512) {
                println("WHAT THE FRACK ARE YOU DOING? Motor #" + str(i)
                        + " is " + str(thrustValues[i]));
            }
            thrustValues[i] = (int) map(thrustValues[i], -512, 512, -256, 256);
        }
    }
    
    else if ((joy1x == 0 && joy1y == 0) && (joy2x != 0)) { //rotation mode
        thrustValues = getRotation(-joy2x);
    }
    
    else if ((joy1x != 0 || joy1y != 0) && (joy2x != 0)) { //strafe and rotation mode
        int[] transValues = getTranslation(joy1y, -joy1x);
        int[] rotValues = getRotation(-joy2x);
    
        for (int i = 0; i < 4; i++) { //this code combines the values
            thrustValues[i] = (int)((transValues[i] + 0.5*rotValues[i]) / 2);
        }
    
        // In the special case of rotating and translating, the rotation control vector will be red
        // . and the translation control vector will be green
        thrustValues[4] = rotValues[4];
        thrustValues[5] = rotValues[5];
    }
    ...
    for (int counter = 0; counter < 4; counter++) {
        ...
        thrustValues[counter] = constrain(thrustValues[counter], 0, 511);
